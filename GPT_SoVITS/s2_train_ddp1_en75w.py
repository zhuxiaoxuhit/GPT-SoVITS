...
#device = "cuda"  # cuda以外的设备，等mps优化后加入
...
def initialize(args, hps):
    ...
    # parameter 'local_rank' is set by torch.distributed.launch
    # or use os.environ['LOCAL_RANK'] directly
    ...
    net_d = MultiPeriodDiscriminator(hps.model.use_spectral_norm).cuda(device) 
    ...
    return local_rank, rank, world_size, net_g, net_d, optim_g, optim_d 

    #run(local_rank, world_size, hps) 

def main():
    args = get_args()
    local_rank, rank, world_size, net_g, net_d, optim_g, optim_d = initialize(args, hps)
