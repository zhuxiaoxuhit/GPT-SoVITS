...
do
    ...
    echo "文件名：$split_file"
    #nohup srun -p speech -n 1 --gpus-per-task 1 --cpus-per-task 5 --quotatype=spot  python process_tts_pipline.py  --start 0 --end -1 --input_text_dir $split_txts_dir --input_lst $split_file --input_wav_dir ${wav_prefix} --out_fold $c1_res_dir >${logs_dir}/$(basename $split_file) 2>&1 &
    ...
    #-j c1-$i \
    #/usr/bin/bash `pwd`/sc_c1.sh  $split_txts_dir $split_file  ${wav_prefix} $c1_res_dir $i >${logs_dir}/$(basename $split_file) 2>&1 & 
...
#find ${out_dir} -type f  -name "*.json" > $c1_res_dir/out.list
