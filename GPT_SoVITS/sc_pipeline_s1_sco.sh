...
if [ $server = "1988" ]; then
    ...
    export LOCAL_RESOURCE_TYPE=N1lS.Ib.I20.8
    export LOCAL_RESOURCE_DESC=8XV100-32GB
...
srun -j s1ddp -p $LOCAL_PARTITION_ID  --workspace-id $LOCAL_WORKSPACE_ID -y -f pt -r N1lS.Ib.I20.8 -N $world_size -d Allreduce sh /mnt/share_afs/share-workspace/zhuxiaoxu/code/GPT-SoVITS/GPT_SoVITS/sc_s1_sco_ddp.sh $config_path


#        #/usr/bin/bash `pwd`/sc_s1_sco.sh  \
#sco acp jobs create \
...
#    --job-name=s1-train \
#    --command="\
#		/usr/bin/bash `pwd`/sc_s1_sco_ddp.sh  \
#		$config_path \
#        $logs_dir
#        "
# 
