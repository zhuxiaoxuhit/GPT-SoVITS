...
python -m torch.distributed.launch --nnodes $WORLD_SIZE --node_rank $RANK --master_addr $MASTER_ADDR --nproc_per_node 8 --master_port $MASTER_PORT  s1_train_ddp.py -c $config_path 


#python -m launch --nnodes $WORLD_SIZE --node_rank $RANK --master_addr $MASTER_ADDR --nproc_per_node 8 --master_port $MASTER_PORT s1_train_ddp.py -c $config_path 
